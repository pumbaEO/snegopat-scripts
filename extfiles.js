...
function СравнитьФайлы (ПутьКФайлу1, ПутьКФайлу2) {
    ...
    // Пока вручную. 
    var РасширенияДляТиповогоСравнения = "mxl, txt, js, vbs";
    if (РасширенияДляТиповогоСравнения.indexOf(ext) > 0) {
        var fc = v8New("СравнениеФайлов")
        fc.ПервыйФайл = ПутьКФайлу1;
        fc.ВторойФайл = ПутьКФайлу2;
        fc.СпособСравнения = СпособСравненияФайлов.Binary;
        if (ext=="mxl") fc.СпособСравнения = СпособСравненияФайлов.ТабличныйДокумент;
        if (ext=="txt") fc.СпособСравнения = СпособСравненияФайлов.TextDocument;
        if (ext=="js") fc.СпособСравнения = СпособСравненияФайлов.TextDocument;
        fc.ПоказатьРазличия()
    } else { //для обработок 
