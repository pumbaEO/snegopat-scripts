...
function macrosОткрытьОкноВнешнихФайлов() {
    ...
    if(!мФормаСкрипта){
        ...
        loadDiffBackends()
        /* registerDiffBackend("mxl", СравнениеФайлов1С);
        registerDiffBackend("txt", СравнениеФайлов1С);
         registerDiffBackend("js", СравнениеФайлов1С); */
        loadDvcsBackends();
...
function КонтекстноеМенюДополнительноСравнитьФайл (Кнопка) {
    var ТекущаяСтрока = мФормаСкрипта.ЭлементыФормы.ДеревоФайлов.ТекущиеДанные;
...
} //КонтекстноеМенюДополнительноСравнитьФайл
...
function КонтекстноеМенюКнDvcsУдалить(Кнопка) {
    ...
    if (ТекущаяСтрока) {
        caller = getDvcsBackendForPath(ТекущаяСтрока.ИмяФайла);
        if (caller!=null) caller("DELETE", ТекущаяСтрока.ИмяФайла, "");
    }
}
    ...
    if (мИспользоватьВерисонирование) {
            for (var key in DvcsBackends) {
                if (DvcsBackends[key]("TEST", лКаталог, "")) {
                    ...
                    var result = DvcsBackends[key]("CATALOGSTATUS", лКаталог, "")
                    break;
                }
            }
...
function dvcsGetMissingFiles (лКаталог, лСтрокаДереваФайлов) {
    if (мИспользоватьВерисонирование) { // получим отсутствующие файлы. Писать будем Имя = путь, т.к. каталога уже может и не существовать. 
            ...
            if(лФайл.ЭтоКаталог() == true)
            {
                ...
                if (мИспользоватьВерисонирование) {
                    ...
                    for (var key in СоответствиеКаталоговИDvcs) {
                        if (лСтрокаДереваФайлов.ИмяФайла.indexOf(key) >=0) {
                            var status = СоответствиеКаталоговИDvcs[key]("FILESTATUS", key, лСтрокаДереваФайлов.ИмяФайла);
                            лСтрокаДереваФайлов.Статус = status;
                            break
                        }
                    }
                }
                ...
                for (var i = 0; i<лСтрокаДереваФайловСтарая.Строки.Количество(); i++) { 
                    ...
                    var МассивФайлов = лТекСтрока.Имя.split('\\')
                        ...
                        while( x<МассивФайлов.length) {
                            ...
                            СтруктураПоиска.insert("ИмяФайла", ПутьДляПоиска);
                            ...
                            if (НайденныеСтроки.Количество() == 0) {
                                ...
                                if(лФайл.ЭтоКаталог() == true)
                                {
                                    if(ValueIsFilled(лФайл.Расширение)) НоваяСтрока.Имя+=лФайл.Расширение
                                }
                                else{
...
function loadDvcsBackends() {
    DvcsBackends = {}
    if (!мИспользоватьВерисонирование) return 
    var mainFolder = profileRoot.getValue("Snegopat/MainFolder")
...
function мЗаписатьНастройки() {
    ...
    profileRoot.setValue(pflVersionControl, мИспользоватьВерисонирование)
...
function НастройкиПриОткрытии() {
    ...
    мФормаНастройки.ИспользоватьВерисонирование = мИспользоватьВерисонирование;
    ...
    мФормаНастройки.ЭлементыФормы.ПанельНастроекВерсионныйКонтроль.Видимость = мИспользоватьВерисонирование;
}
...
function ИспользоватьВерисонированиеПриИзменении(Элемент) {
    мФормаНастройки.ЭлементыФормы.ПанельНастроекВерсионныйКонтроль.Видимость = мФормаНастройки.ИспользоватьВерисонирование;
}
