...
function мДобавитьФайлы(пПуть, пУзел) {
    ...
    for (var лИнд=0; лИнд<лФайлы.Количество(); лИнд++)
    {
        ...
        if ((мПоказыватьРезультатПоискаСписком) && (мФормаСкрипта.Фильтр != '')){  //Тут сделаем свое добавление, а старый вариант оставим.
            ...
            if (лФайл.ЭтоКаталог() == false) {
                ...
                if(мОтображатьРасширениеФайлаВПредставлении == true){ //а тут хитро, в теории у нас родитель только один с корневым каталогом, вот этот корневой каталог и вырежем из путей.
