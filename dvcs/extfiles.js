...
function registerDVCSBackend(description, caller) {
    ...
    if (мФормаСкрипта!=null) мОбновитьФайлы();
} //registerDVCSBackend
...
function ДобавитьКаталоги(пТзКаталоги)
{
    ...
    for (var лИнд=0; лИнд<пТзКаталоги.Количество(); лИнд++)
    {
        ...
        лСтрокаДереваФайлов.ЭтоКаталог = true
...
function КоманднаяПанельDVCSСравнитьСПоследнейВерсией(Элемент)
{
...
function GetCaller(лКаталог) {
    if (лКаталог == undefined) return null
    return DvcsBackends["fossil"]
    
    
}

function КоманднаяПанельDVCSОткрытьТекущийКаталогВКоманднойСтроке(Элемент) {
    ...
    caller = GetCaller(лКаталог)
    if (caller!=null) return caller("RUN", лКаталог, лТекСтрока.ИмяФайла)
    //ЗапуститьПриложение("cmd.exe", лКаталог);
} //КоманднаяПанельDVCSОткрытьТекущийКаталогВКоманднойСтроке

function КоманднаяПанельDVCSДействиеДобавить(Элемент) {
    ...
    caller = GetCaller(лКаталог)
    if (caller!=null) return caller("ADD", лКаталог, лТекСтрока.ИмяФайла)
    
}

function КоманднаяПанельDVCSДействиеИстория(Элемент) {
    return 
}
function КоманднаяПанельDVCSДействиеCommit(Элемент) {
    return
}
